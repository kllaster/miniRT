...
float			parse_float(char **str)
{
	...
	if (*(*str) == '.' && (++(*str)))
	{
		...
		num += (mantissa / powf(10.0, ft_numlen(mantissa)));
	}
	return (num);
}
...
t_coordinates	*parse_coordinates(char **str)
{
	...
	s_coordinates->x = parse_float(str);
	check_next_arg(str);
	s_coordinates->y = parse_float(str);
	check_next_arg(str);
	s_coordinates->z = parse_float(str);
...
t_angle			*parse_angle(char **str)
{
	...
	s_angle->x = parse_float(str);
	check_next_arg(str);
	s_angle->y = parse_float(str);
	check_next_arg(str);
	s_angle->z = parse_float(str);
	return (s_angle);
}
