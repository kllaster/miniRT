...
void	inter_triangle(t_triangle *s_triangle, t_ray *s_ray)
{
	...
	s_ray_inter.s_vec_inter = vec_cross_product(&s_ray->s_vec_start_dir, s_triangle->s_vec_edge_2);
	det = vec_scalar_mul(s_triangle->s_vec_edge_1, &s_ray_inter.s_vec_inter);
	if (det > -MIN_DISTANCE && det <= MIN_DISTANCE)
		return ;
	det = 1 / det;
	...
	if (a < 0 || a > 1)
		return ;
	s_ray_inter.s_vec_inter_dir = vec_cross_product(&s_ray_inter.s_vec_inter_dir, s_triangle->s_vec_edge_1);
	b = vec_scalar_mul(&s_ray->s_vec_start_dir, &s_ray_inter.s_vec_inter_dir) * det;
	if (b < 0 || a + b > 1)
		return ;
	det = vec_scalar_mul(s_triangle->s_vec_edge_2, &s_ray_inter.s_vec_inter_dir) * det;
	if (det < MIN_DISTANCE || det > s_ray->length)
		return ;
