...
void 	inter_plane(t_plane *s_plane, t_ray *s_ray)
{
	...
	x = vec_scalar_mul(&s_ray->s_vec_start_normal, s_plane->s_vec_dir);
	if (ft_dabs(x) <= 0.0001)
		return ;
	...
	s_ray_inter.s_vec_inter = vec_mul(&s_ray->s_vec_start_normal, x);
	s_ray_inter.s_vec_inter = vec_sum(&s_ray->s_vec_start, &s_ray_inter.s_vec_inter);
	s_ray_inter.s_vec_inter_normal = *s_plane->s_vec_dir;
	*s_ray = s_ray_inter;
}