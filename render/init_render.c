...
void	get_another_camera(t_rt *s_rt)
{
	...
	if (s_rt->s_stage.s_selected_camera == NULL)
		i = 1;
	else
		i = s_rt->s_stage.s_selected_camera->count;
	s_list_cameras = s_rt->s_stage.s_list_cameras;
	while (s_list_cameras)
	{
		if (((t_camera *)s_list_cameras->content)->count < i)
		{
			s_rt->s_stage.s_selected_camera = s_list_cameras->content;
			s_rt->s_stage.s_selected_camera->count++;
			if (s_rt->s_stage.s_selected_camera->init == 0)
				init_camera(s_rt, s_rt->s_stage.s_selected_camera);
			return ;
		}
		s_list_cameras = s_list_cameras->next;
	}
	s_rt->s_stage.s_selected_camera = s_rt->s_stage.s_list_cameras->content;
	s_rt->s_stage.s_selected_camera->count++;
}
