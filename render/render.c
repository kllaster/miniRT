...
t_rgb		get_color_pixel(t_thread_data *s_thread_data, t_ray *s_ray)
{
	...
	s_color_light = rgb_mul(s_thread_data->s_ambient_light.s_color,
...
int			anti_aliasing(t_thread_data *s_thread_data, int x, int y, t_ray *s_ray)
{
	...
	{
		get_norm_start_ray(x + s_thread_data->s_aa_sample.matrix_sample[count_rays][0],
					 y + s_thread_data->s_aa_sample.matrix_sample[count_rays][1],
					 s_ray, s_thread_data->s_selected_camera);
